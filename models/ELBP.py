...
class ELBP:
    ...
    def __init__(self, windowX=8, windowY=8, p=16, r=2):
        self.name = "elbp"
    ...
    def compute_features(self, image):
        ...
        # Computes histograms for each window in the elbp matrix
        for y in range(0, height, self.windowY):
            currR = lbp[y:min(height, y+self.windowY), :]
            for x in range(0, width, self.windowX):
                window = currR[:, x:min(width, x+self.windowX)]
                feature.append(np.histogram(
                    window.ravel(), bins=self.p+2, range=(0, self.p + 1))[0])
    ...
    def get_var_mat(self, image):
        ...
        min_var = np.min(elbp)
        max_var = np.max(elbp)
        elbp = ((elbp-min_var)*20)//(max_var-min_var)
        elbp = np.where(elbp > 19, 19, elbp)
        return elbp

    def compute_elbp_feature(self, image):
        h, w = 32, 32
        elbp = self.get_lbp(image)
        var = self.get_var_mat(image)
