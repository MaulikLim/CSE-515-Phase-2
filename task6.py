...
def group_by_type(labels, metrics):
    type_metrics = {}
...
if tech == 'pca':
    ...
    types, original_metrics = group_by_type(labels, original_metrics)
    ...
    for ind, d in enumerate(original_metrics):
        distance_score = np.sum(np.abs(d - q_feature_mat))
        result.append([types[ind], distance_score])
